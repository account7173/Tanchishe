...
GameLog logs[100]; // 可以存储最多 100 条日志
...
void pause()
{
    ...
    while (1)
    {
        ...
        { // 如果按下 F5，则显示日志
            int currentDuration = (int)difftime(time(NULL), pauseStart) +
                                  logs[logCount].duration;
            system("cls");
            printf("当前游戏会话信息：\n");
            printf("用户名: %s\n 得分: %d\n 游戏开始时间: %s 游戏持续时间: %d 秒\n",
...
void displayGameLogs()
{
    ...
    {
        printf("%d. 用户名: %s, 得分: %d, 游戏开始时间: %s, 持续时间: %d 秒\n",
               i + 1, logs[i].username, logs[i].score, ctime(&logs[i].startTime),
               logs[i].duration);
    }
    system("pause"); // 暂停查看日志
    system("cls");   // 清屏后返回游戏
...
void gamestart(const char *username)
{
    ...
    logs[logCount].score = 0;    // 初始得分为 0
    logs[logCount].duration = 0; // 初始持续时间为 0
}
void endgame()
{
